...
(
	-- партицируем начиная с 1 января 2023 года включительно
...
SET optimizer = on; -- включаем GPORCA оптимизатор
EXPLAIN
SELECT pr.product_name, SUM(pr.price * s.cnt) AS sum_of_sale
FROM sales AS s
JOIN product AS pr ON s.product_id = pr.product_id
-- при таком условии с датами оптимизатор возьмёт конкретную партицию за первый месяц
-- но если написать "EXTRACT(MONTH FROM sales_date) = 1" пройдёт по всем партициям!
